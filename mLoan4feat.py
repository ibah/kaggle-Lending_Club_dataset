...
#loan_issued_monthly = data.groupby(pd.TimeGrouper(freq='M')).size() <- this is very slow
...
#for status in statuses:
#    ls = '-' if status in default else ':'
#    data[data.loan_status==status].groupby(
#        pd.TimeGrouper(freq='M')).size().plot(
#        label=status, ls=ls, lw=2, logy=True)
#plt.legend(bbox_to_anchor=(0, 1, 1, 0), loc=3,
#           ncol=2, mode="expand")
...
#status_groups = data.groupby('loan_status')
#tmp = status_groups.loan_amnt.apply(lambda x: x.values)
#fig, ax = plt.subplots(1)
#ax.boxplot(tmp, labels=tmp.index, patch_artist=True)
#fig.autofmt_xdate()
fig, ax = plt.subplots(1)
sns.boxplot('loan_status', 'loan_amnt', data=data, palette='pastel')
fig.autofmt_xdate()

# box plot: status -> last_pymnt_d
# 1
fig, ax = plt.subplots(1)
sns.boxplot(data.loan_status, np.log1p(data.last_pymnt_amnt), palette='pastel')
fig.autofmt_xdate()
# 2
colors = make_binary_label(data.loan_status)
fig, ax = plt.subplots(1)
sns.boxplot(data.loan_status, np.log1p(data.last_pymnt_amnt),
            hue=colors, palette='pastel')
plt.legend(bbox_to_anchor=(.8, -.2), loc=2)#, borderaxespad=0.)
fig.autofmt_xdate()



# Analyse these top features:
#52                                last_pymnt_d  1.555484e-01 <- this variable doesn't make sense
#24                     collection_recovery_fee  1.312749e-01
#23                                  recoveries  1.219645e-01
#20                             total_rec_prncp  9.074021e-02
date_columns = data[date].apply(pd.to_datetime, format='%b-%Y').dropna()

statuses
sns.distplot(date_columns.last_pymnt_d.astype(np.int64))
for status in statuses:
    ...
    # status = 'Issued' negative / missing correct
    # 'Late (31-120 days)' nice multi camel
    # 'Default' middile sipke
    # 'Current' U
    # 'Charged Off' 0
    # 'Fully Paid' 0
    plt.hist(date_columns.last_pymnt_d[data.loan_status==status].astype(np.int64))
    sns.kdeplot(date_columns.last_pymnt_d[data.loan_status==status].astype(np.int64))


# there's a lot of cheating in the data

# last_paymnt_d has very strange properties - try to plot it

# collection recovery fee - it is being filled in after default, not before - we can't use it
...
data[data.recoveries>0].loan_status.unique()
# ['Charged Off', 'Does not meet the credit policy. Status:Charged Off']

# total_rec_prncp - this is the first real variable that we can use
# plot it
